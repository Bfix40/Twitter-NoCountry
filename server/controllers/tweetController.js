      ...
      if (foundedForbiddenWords.length > 0 && !confirmation) {
        ...
        return res
          ...
          .json({
            ...
            confirmationMessage,
            forbiddenWords: foundedForbiddenWords,
          });
      }
    }
...
const getAllTweets = async (_req, res) => {
  try {
    const tweets = await Tweet.find()
      .populate("author", "name image username email confirmed")
    ...
    const allTweets = [...tweets, ...retweets].sort(
...
const getLikedTweetsByUserId = async (req, res) => {
  try {
    ...
    const likes = await Tweet.find({ likes: userId })
      .populate("author", "name image username email confirmed")
      .populate("likes", "name image username email")
      .sort({ createdAt: -1 });
...
const createRetweet = async (req, res) => {
  try {
    ...
    if (alreadyReTweeted) {
      return res
        .status(400)
        .json({ message: "El usuario ya ha retwitteado este tweet" });
    }
...
const deleteRetweet = async (req, res) => {
  try {
    ...
    if (!deletedRetweet) {
      return res.json({
        message: "No se encontró el retweet generado por el usuario",
      });
    }
...
const getAllFollowsTweets = async (req, res) => {
  try {
    ...
    const tweets = await Tweet.find({ author: { $in: followedUserIds } })
      .populate("author", "name image username email confirmed")
      .populate("likes retweets", "name image username email");

    const retweets = await Retweet.find({ author: { $in: followedUserIds } })
      .populate({
        path: "author",
        select: "name image username email confirmed",
      })
